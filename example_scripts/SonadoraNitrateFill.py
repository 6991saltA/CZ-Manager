...
fa = Featureactions.objects.filter(featureactionid=1).get()
NO3uMresult = Timeseriesresults.objects.get(resultid=1)
NO3NO3mgLresult = Timeseriesresults.objects.get(resultid=2)
Abs254nmresult = Timeseriesresults.objects.get(resultid=3)
Abs350nmresult = Timeseriesresults.objects.get(resultid=4)
censorcode = CvCensorcode.objects.filter(name="Sona").get()
qualitycodegood = CvQualitycode.objects.filter(name="Quality Code").get()
reader = csv.reader(f)
lastrow=None
i=0
insertRecord = False
